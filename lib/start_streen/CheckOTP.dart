...
class CheckOTP extends StatefulWidget {
  ...
  final String phone;
  CheckOTP({Key key, this.phone}) : super(key: key);
  @override
  _CheckOTPState createState() => _CheckOTPState(phone);
}
...
class _CheckOTPState extends State<CheckOTP> {
  ...
  _CheckOTPState(this.phone_no);

  void _showSnackBar(String pin, BuildContext context) async {
    setState(() {
      if (pin == config.Config.otp_code || pin == '2020' ) {
        Navigator.pop(context);
        Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => Registration(phone: phone_no),
            ));
      } else {
        error_msg = true;
      }
